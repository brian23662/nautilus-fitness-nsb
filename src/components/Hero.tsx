...
// Local hero video — placed at /public/hero.mp4 by the user.
// Files in /public are served from the site root (e.g. /hero.mp4).
  ...
  return (
    ...
    >
      ...
      <video
        ...
        // Poster image shown while the video buffers / before it autoplays.
        // Files in /public/images/ are served from /images/.
        poster="/images/hero-woman-sled.png"
      >
        <source src={HERO_VIDEO} type="video/mp4" />
      </video>

      {/* Cinematic gradient overlay — lightened in the middle so the
          video is more visible behind the logo, with darker bands at
          top (under nav) and bottom (behind headline/buttons) for legibility. */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-ink/60 via-ink/10 to-ink/95" />
      {/* Side vignette — subtler now so it doesn't fight the video */}
      ...
      <div className="relative z-10 flex w-full flex-col items-center px-6 pb-24 pt-28 text-center md:pt-32">
        {/* Eyebrow — sits directly above the logo so it always
            stays visually attached, regardless of viewport height. */}
        <motion.p
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.4 }}
          className="mb-5 font-mono text-[0.65rem] tracking-[0.32em] text-sky"
        >
          ◆ NEW SMYRNA BEACH, FL ◆
        </motion.p>
